...
export class Grai170 extends Epc<Grai170> {
	...
	static TAG_URI_TEMPLATE = (filter: number, company: string, asset: string, serial: string) => {return `urn:epc:tag:${this.TAG_URI}:${filter}.${company}.${asset}.${serial}`};
	// C.A.S   (Company, AssetType, Serial)
	static PID_URI_TEMPLATE = (company: string, asset: string, serial: string) => {return `urn:epc:id:grai:${company}.${asset}.${serial}`}; 
	...
	clone() {
		return new Grai170().setFromBitArray(this) as Grai170;
	}
