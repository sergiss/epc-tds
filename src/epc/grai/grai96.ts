...
export class Grai96 extends Epc<Grai96> {
	...
	static TAG_URI_TEMPLATE = (filter: number, company: string, asset: string, serial: number) => {return `urn:epc:tag:${this.TAG_URI}:${filter}.${company}.${asset}.${serial}`};
	// C.A.S   (Company, AssetType, Serial)
	static PID_URI_TEMPLATE = (company: string, asset: string, serial: number) => {return `urn:epc:id:grai:${company}.${asset}.${serial}`}; 
	...
	clone() {
		return new Grai96().setFromBitArray(this) as Grai96;
	}
