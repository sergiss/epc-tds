...
export class Gsrn96 extends Epc<Gsrn96> {
	...
	static TAG_URI_TEMPLATE = (filter: number, company: string, service: number) => {return `urn:epc:tag:${this.TAG_URI}:${filter}.${company}.${service}`};
	// C.S (Company, Service)
	static PID_URI_TEMPLATE = (company: string, asset: number) => {return `urn:epc:id:gsrn:${company}.${asset}`}; 
	...
	clone() {
		return new Gsrn96().setFromBitArray(this) as Gsrn96;
	}
	...
	setServiceReference(value: number) {
