...
export class Cpi96 extends Epc<Cpi96> {
	...
	static TAG_URI_TEMPLATE = (filter: number, company: string, part: number, serial: number) => {return `urn:epc:tag:${this.TAG_URI}:${filter}.${company}.${part}.${serial}`};
	// C.P.S   (Company, Part, Serial)
	static PID_URI_TEMPLATE = (company: string, part: number, serial: number) => {return `urn:epc:id:cpi:${company}.${part}.${serial}`};
	...
	clone() {
		return new Cpi96().setFromBitArray(this) as Cpi96;
	}
