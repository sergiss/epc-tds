...
export class Sscc96 extends Epc<Sscc96> {
	...
	static TAG_URI_TEMPLATE = (filter: number, company: string, serial: string) => {return `urn:epc:tag:${this.TAG_URI}:${filter}.${company}.${serial}`};
	// C.S   (Company, Serial)
	static PID_URI_TEMPLATE = (company: string, serial: string) => {return `urn:epc:id:sscc:${company}.${serial}`};                       
	...
	clone() {
		return new Sscc96().setFromBitArray(this) as Sscc96;
	}
	...
	setSerialReference(value: number) {
