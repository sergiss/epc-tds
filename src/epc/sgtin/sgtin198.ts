...
export class Sgtin198 extends Epc<Sgtin198> {
	...
	static TAG_URI_TEMPLATE = (filter: number, company: string, item: string, serial: string) => {return `urn:epc:tag:${this.TAG_URI}:${filter}.${company}.${item}.${serial}`};
	// C.I.S   (Company, Item, Serial)
	static PID_URI_TEMPLATE = (company: string, item: string, serial: string) => {return `urn:epc:id:sgtin:${company}.${item}.${serial}`};
	...
	clone() {
		return new Sgtin198().setFromBitArray(this) as Sgtin198;
	}
