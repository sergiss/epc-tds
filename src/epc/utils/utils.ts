 ...
 */
const computeCheckDigit = (barcode: string): number => { // https://www.gs1.org/services/how-calculate-check-digit-manually
 ...
 */
const getMaxValue = (bits: number): number => {
 ...
 * @returns 
 */
const hexToByte = (hex: string, offset: number): number => {
  return (
    (DEC_TABLE[hex.charCodeAt(offset)] << 4) |
 ...
 */
const randomEan = (n: number): string => {
  let result = "";
  for (let i = 1; i < n; ++i) {
