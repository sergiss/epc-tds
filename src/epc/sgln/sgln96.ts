...
export class Sgln96 extends Epc<Sgln96> {
	...
	static TAG_URI_TEMPLATE = (filter: number, company: string, location: string, extension: number) => {return `urn:epc:tag:${this.TAG_URI}:${filter}.${company}.${location}.${extension}`}; 
	// C.L.E   (Company, Location, Extension)
	static PID_URI_TEMPLATE = (company: string, location: string, extension: number) => {return `urn:epc:id:sgln:${company}.${location}.${extension}`}; 
	...
	clone() {
		return new Sgln96().setFromBitArray(this) as Sgln96;
	}
