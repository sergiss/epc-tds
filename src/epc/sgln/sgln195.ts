...
import { Sgln96 } from "./sgln96";
import Utils from "../utils";

export class Sgln195 extends Epc<Sgln195> {
	...
	static TAG_URI_TEMPLATE = (filter: number, company: string, location: string, extension: string) => {return `urn:epc:tag:${this.TAG_URI}:${filter}.${company}.${location}.${extension}`};
	 // C.L.E   (Company, Location, Extension)
	static PID_URI_TEMPLATE = (company: string, location: string, extension: string) => {return `urn:epc:id:sgln:${company}.${location}.${extension}`};
	...
	clone() {
		return new Sgln195().setFromBitArray(this) as Sgln195;
	}
