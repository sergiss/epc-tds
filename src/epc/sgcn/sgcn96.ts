...
export class Sgcn96 extends Epc<Sgcn96> {
	...
	static TAG_URI_TEMPLATE = (filter: number, company: string, coupon: string, serial: string) => {return `urn:epc:tag:${this.TAG_URI}:${filter}.${company}.${coupon}.${serial}`};
	// C.C.S   (Company, Coupon, Serial)
	static PID_URI_TEMPLATE = (company: string, coupon: string, serial: string) => {return `urn:epc:id:sgcn:${company}.${coupon}.${serial}`}; 
	...
	clone() {
		return new Sgcn96().setFromBitArray(this) as Sgcn96;
	}
	...
	setCouponReference(value: number) {
