...
class Giai96 extends Epc {
  ...
  static TAG_URI_TEMPLATE = (filter, company, asset) => {
    return `urn:epc:tag:${this.TAG_URI}:${filter}.${company}.${asset}`;
  }; // F.C.A (Filter, Company, Asset)
  static PID_URI_TEMPLATE = (company, asset) => {
    return `urn:epc:id:giai:${company}.${asset}`;
  }; // C.A (Company, Asset)

  // Partition table columns: Company prefix, Asset Type
  static PARTITIONS = [
    new Partition(Giai96.PARTITION_END, 40, 12, 42, 13), // 0 40 12 42 13
